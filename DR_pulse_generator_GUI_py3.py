...
class Ui_Dialog_First_Window(object):
    def setupUi(self, Dialog):
        ...
        self.gridLayout = QtWidgets.QGridLayout(Dialog) #grid layout for different widgets
        self.gridLayout.setObjectName("gridLayout") #name of the grid layout
        ...
        self.band_select = QtWidgets.QComboBox(Dialog) #button and popup list
        ...
        self.band_select.addItems(["Low (8.7-13.5 GHz)", "Medium (13.5-18.3 GHz)", "High (18.0-26.5 GHz)"]) #this is the drop down menu
        self.band_select.setCurrentIndex(2) # high band as default
        self.band_select.activated.connect(self.band_change)
        self.gridLayout.addWidget(self.band_select, 0, 0, 1, 2) #row, column, hieght, width (2 wide)
        ...
        self.use_defaults_cb.stateChanged.connect(self.use_defaults) #will gray out options if checked
        ...
        self.font_plus_button.clicked.connect(partial(self.font_plus,Dialog)) #make it so that the argument is evaluated with partial
        ...
        self.gridLayout.addWidget(self.font_minus_button, 0, 5, 1, 1) #this block changes the font size

        self.gridLayout.addWidget(QHLine(), 1, 0, 1, 6) #make a horizontal line
    ...
    def use_defaults(self):
        defaults_decision = self.use_defaults_cb.isChecked() #checks if it is checked

        if defaults_decision == False:
            self.chirp_start_input.setEnabled(True) #ungrays out the textboxes
        ...
        if defaults_decision == True:
            self.chirp_delay_input.setText("0.6") #puts in default values and grays out the values
            ...
            if (band=="High (18.0-26.5 GHz)"): #changes based on what band you are 
            ...
            self.chirp_stop_input.setEnabled(False) #graying out another box
    ...
    def browse_export(self):
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName() #opens a new window to select a file 
        if fileName:
            self.file_export_input.setText(fileName) #set for the outputname
            self.generate_pulse_button.setEnabled(True) #ungray
            self.generate_pulse_button.setFocus() #set focus
    ...
    def generate_pulse(self): # It will eventually do the thing, by which I mean it will probably eventually pass the thing off to a worker thread.
        ...
        sample_rate = 10.0 #our arb only has this output rate, constant of the hardware 
        DR_decision = self.use_DR_cb.isChecked() #for double reasonance pulse
        ...
        try:
            ...
            return 0  #exit this function and dont continue till it is corrected
        ...
        try:
            Ch1_off = float(self.marker_off_input.text()) #generating new inputs and then convert to float
        except:
        ...
        if (Ch1_on > total_waveform_time) or (Ch1_off > total_waveform_time): #for when this trigger starts
            self.error_message = "Trigger start and stop times should be less than the total waveform time!"
            self.raise_error()
            self.marker_on_input.setFocus()
            return 0

        if self.file_export_input.text() == '': #did not give it a valid file
        ...
        chirp_name = self.file_export_input.text() #reading what is in the textbox
        band = self.band_select.currentText()  #set to what you have it as currently
        Width = Overall_Chirp_Stop - Overall_Chirp_Start 

        if DR_decision: #if we want a double reasonance pulse
            try:
                Sinc_Cent_Freq = float(self.sinc_cent_freq_input.text()) #generating a local variable based on what the user put in
            except:
                self.error_message = "DR frequency should be a float!"
                self.raise_error() #raises the error
                self.sinc_cent_freq_input.setFocus() #setting focus on what needs to be fixed
                return 0
            ...
            gap = sinc_start + (Sinc_Window/2) #have done everything neccessary for the sinc position and edges

            if (band == "Low (8.7-13.5 GHz)"):
                if (Sinc_Cent_Freq < 8700.0) or (Sinc_Cent_Freq > 13500.0): #outside of this range
                    self.error_message = "DR frequency is not in lower band range (8700 - 13500 MHz)!"
                    self.raise_error()
                    self.sinc_cent_freq_input.setFocus()
                    return 0
                PDRO = 13600 #this is set for the hardware 
        ...
        waveform_points = int(numpy.ceil((total_waveform_time*10**-6)*(sample_rate*(10**9)))) #setting the waveform points, length of the list
        ...
        if DR_decision == False: #DR decision is a local
            sinc_exists = False
        else:
            sinc_exists = True

        marker = marker1(Ch1_on,sample_rate,Ch1_off,waveform_points)
        pulse_no_sinc = chirp_waveform(Chirp_Delay,sample_rate,Overall_Chirp_Start,Width,Chirp_Duration,waveform_points) #makes the waveform with no sinc
        ...
        if (sinc_exists == False): # just a chirp, no DR pulse
            ...
            pulse = numpy.add(pulse_no_sinc,sinc) #element by element addition, both the same length 

        awg_data=numpy.column_stack((pulse,marker,marker)) #generating a 3col array by concatenation
    ...
    def plotter(self): 
        rcParams.update({'figure.autolayout': True}) # Magic from here: https://stackoverflow.com/questions/6774086/why-is-my-xlabel-cut-off-in-my-matplotlib-plot, fixes xlabel
        self.plot = Actual_Plot() #made window for the plot, creating a new plot object
        self.plot.shortcut = QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+W"), self.plot) #close the window by keyboard
        self.plot.shortcut.activated.connect(self.plot.close) #recieves this keyboard command, it will close
        self.plot.alt_shortcut = QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+Q"), self.plot)
        self.plot.alt_shortcut.activated.connect(self.plot.close) #another shortcut for exiting out of a window
        self.plot.show() #actually show the plot
...
class Actual_Plot(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        m = WidgetPlot(self) #actually plotting the widget
        vlay.addWidget(m) #adding it into the layout
...
class PlotCanvas(FigureCanvas):
    ...
    def plot(self):
        ...
        ax.plot(marker) #these need to be global so u dont need ot pass through all of these
        ...
        ax.xaxis.set_major_formatter(ticks_x)  #these are all graphical interface stuff

        ax.set_title('Arb and Trigger Pulse')
        ax.set_xlabel('Time (microseconds)')
        ax.set_ylabel('FID Amplitude (arb. units)')
        self.draw() #actually draw this on your plot
...
def marker1(Ch1_on,sample_rate,Ch1_off,waveform_points):
    ...
    return out_marker #has to be the same lenght as the total waveform

...
class Ui_Dialog_First_Window(object): #removes from the time domain, for the spurs and does it before the fourier transform
    ...
    def browse(self):
        fileName, _ = QtWidgets.QFileDialog.getOpenFileName() #opening a file explorer
        if fileName:
            self.file_import_input.setText(fileName)
            self.plot_button.setEnabled(False)
            self.are_we_there_yet() #go through the checks
    ...
    def load_input(self):
        try:
            ...
            self.raise_error()
            self.sample_rate_input.setFocus() #raise error if it is not a float and change focus
            return 0
...
# It also tries to figure out what the next best step is to do and directs the focus there to help guide the user.
    def are_we_there_yet(self):
        ...
        if use_full_FID:
            self.gate_start_input.setEnabled(False) #gray out
            self.gate_stop_input.setEnabled(False)
        else:
            self.gate_start_input.setEnabled(True) #dont gray out
            self.gate_stop_input.setEnabled(True)

        if self.file_export_input.text() != '':
            have_export_file = True #so then u have a file
        else:
            have_export_file = False

        if self.file_import_input.text() != '':
            have_data_file = True #so then u have a data file
        else:
        ...
        if have_data_file == False:
            self.browse_import_button.setFocus()
            self.load_button.setEnabled(False) #graying out some of your buttons
            self.plot_button.setEnabled(False) 
            self.extract_spurs_button.setEnabled(False) #guiding the user
            return False
        else: # we have a data file
            if data_loaded == False:
                self.load_button.setEnabled(True) #ungray this button
                self.load_button.setFocus() #set its focus
                self.plot_button.setEnabled(False) #gray out these
                self.extract_spurs_button.setEnabled(False)
                return False
            else: # the data file has been loaded
                if have_export_file == False:
                    self.browse_export_button.setFocus() #set the focus
                    self.extract_spurs_button.setEnabled(False) #gray out button
                    return False
                else: # we have an export filename
                    self.extract_spurs_button.setEnabled(True) #ungray button 
                    self.extract_spurs_button.setFocus()
                    return True #return true, when it reaches this place everything is ready to go
    ...
    def extract(self):
        ...
        thread = self.thread = QtCore.QThread() #math section
...
# Thanks go to this thread: https://gis.stackexchange.com/questions/64831/how-do-i-prevent-qgis-from-being-detected-as-not-responding-when-running-a-hea
    ...
    def calculate_progress(self,percentage_new):
        ...
        if percentage_new > self.percentage:
            ...
            self.progress.emit(self.percentage) #send out signal to the progress bar

...
import numpy as np
#from mylength import mylength, findnonzeros
import math as math
from copy import *
...
# updateseriessquare.m:132
    for i in np.arange(1, np.size(s['column1']['fs']) - 1).reshape(-1):
...
# updateseriessquare.m:368
    ...
    if s['closedout'] == 0:
        s['corners'][0][s['numlines'] + 1]=bestcorner
...
# updateseriessquare.m:380
        s['alineorder'][s['nextrow']-1,s['nextcolumn']-1]=s['numlines'] + 1
# updateseriessquare.m:381
        s['alineorder'][s['nextnextrow']-1,s['nextnextcolumn']-1]=s['numlines'] + 2
...
# updateseriessquare.m:388
        s['allpredicts'][s['nextrow']-1,s['nextcolumn']-1]=s['nextpredictf']
...
def hashfromsquare(s):
    ...
    for i in np.arange(0,numrows).reshape(-1):
        thisrow=fgrid[i,:]
    ...
    for i in np.arange(0,numrows).reshape(-1):
        for j in np.arange(0,4).reshape(-1):
...
# updateseriessquare.m:655
    g2[np.arange(),1]=g[np.arange(),4]
# updateseriessquare.m:656
    g2[np.arange(),2]=g[np.arange(),3]
# updateseriessquare.m:657
    g2[np.arange(),3]=g[np.arange(),2]
# updateseriessquare.m:658
    g2[np.arange(),4]=g[np.arange(),1]
    return g2
# updateseriessquare.m:659


output=updateseriessquare(s)
print("done!!! :))) ")
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  9 09:21:34 2020

@author: rodri
"""
import numpy as np

def settingsfromtightness(t):
    ...
    ts['highsidetolerance'] =  np.array([50, 50, 10])*0.5*t  #how well can we predict the next f? lactone had to lower
    ts['lowsidetolerance'] =   np.array([300, 70, 25])*t
    ts['lowjthresh'] = 7;
    ...
    ts['ladderSearchtimes'] = np.array([[5,1e-40],[20, 1e-40],[100,1e-40],[500,1e-8],[4000,100]]); #these are time,pval pairs. last one means "just quit after 1000".
    ts['seriesaratio'] = 15;
    ts['seriesbratio'] = 15;
    ...
    ts['lines'] = np.arange(1,51)
    if t > 1:
        ts['lines'] = np.arange(1,61)
    if t > 2:
        ts['lines'] = np.arange(1,81)
   ...
   ## ts['isotopefitting'] = isotopefitting; # i highlighted this=!
   ...
   ## ts.patternfitting = patternfitting;
    ...
    variant3['bendTolerance'] = np.array([50, 20, 10, 10, 10, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]); 
    #third line in f1 a series can be how far out of straight
    variant3['lowj'] = 5;  #all tolerances looser down here
    variant3['lines'] = np.arange(1,31)
    ...
    tightnesslist['ts']=ts
    tightnesslist['bowties']=bowties
    tightnesslist['variant3']=variant3
    tightnesslist['bruteforce']=bruteforce
    tightnesslist['isotopefitting']=isotopefitting
    tightnesslist['patternfitting']=patternfitting
    return tightnesslist
